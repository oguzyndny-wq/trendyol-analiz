...
if baslat_btn:
    ...
    if amz_sip_file and amz_mal_file:
        try:
            ...
            st.session_state['amz_sip_adet'] = len(df_as) # Rapor satır sayısı (Eşsiz ASIN miktarı)
...
if st.session_state['hesaplandi_ty'] or st.session_state['hesaplandi_amz']:
    ...
    cc3.metric("🛒 Trendyol Sipariş Başı Kâr (Müşteri Bazlı)", "₺{:.2f}".format(st.session_state['ty_kar'] / st.session_state['ty_sip_adet'] if st.session_state['ty_sip_adet'] > 0 else 0.0))
